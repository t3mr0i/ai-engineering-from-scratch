...
# NOTE: "Products & Value Streams" (col 11) was dissolved and merged into
# "Corporate Functions" — the sheet still carries the legacy column, but the
# build folds it into Corporate Functions with max() so generated output stays
# merged. Do not re-add a separate PVS role without updating catalog.json too.
ROLE_COLS = {
...
# Legacy roles folded into another role at build time: {source: target}.
MERGED_ROLES = {
    "Products & Value Streams": "Corporate Functions",
}
...
def main():
    ...
    # Fold dissolved roles into their merge target (max level wins).
    for source, target in MERGED_ROLES.items():
        for cid, lvl in role_targets.get(source, {}).items():
            current = role_targets[target].get(cid)
            if current is None or LEVEL_ORDER[lvl] > LEVEL_ORDER[current]:
                role_targets[target][cid] = lvl
        role_targets.pop(source, None)

    roles = [
        {"name": name, "targets": dict(sorted(role_targets[name].items()))}
        for name in ROLE_COLS.values()
        if name not in MERGED_ROLES
    ]
