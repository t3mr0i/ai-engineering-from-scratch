...
Ported from the standalone **[llm-tutorial](https://github.com/nachtgold/llm-tutorial)**
(MIT © 2026 nachtgold) into the LHIND AI Learning Catalog. The original ships
as Vercel serverless functions with an optional multiplayer "journey mode"
(Vercel Blob + Edge Config + bcryptjs admin console). That backend does not
exist on this gated static site, so the primer runs **solo-only** here:
progress is stored per-device in `localStorage`, exactly like the upstream
app without a database. All 16 mini-games, the glossary, the quiz and the
reflection run fully client-side.
...
- **Solo-only.** In `app.js`, `api()` is a no-op (no backend), the cover CTA
  is just the start button (no journey join / admin), and the admin button is
  null-guarded. Journey/admin/lobby code remains but never triggers.
