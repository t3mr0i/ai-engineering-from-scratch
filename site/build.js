#!/usr/bin/env node
/**
 * Build script for the LHIND AI Learning Catalog website.
...
function build() {
  ...
  // Visible set is the single source of truth for everything user-facing.
  // PHASES keeps all phases (so deep links to hidden lessons still resolve);
  // counts, sitemap, llms.txt, and the rendered site all derive from visible.
  const visiblePhases = phases.filter(p => !p.hidden);
  const visiblePhaseIds = new Set(visiblePhases.map(p => p.id));
  const visibleArtifacts = artifacts.filter(a => visiblePhaseIds.has(a.phase));

  // Stats — count only what's reachable on the site.
  let totalLessons = 0;
  ...
  console.log(`\n📊 Stats (visible / total):`);
  console.log(`   Phases: ${visiblePhases.length} / ${phases.length}`);
  console.log(`   Lessons: ${totalLessons} / ${phases.reduce((a, p) => a + p.lessons.length, 0)}`);
  console.log(`   Complete: ${completeLessons}`);
  console.log(`   Summaries: ${summarized}, Keywords: ${withKeywords}`);
  console.log(`   Glossary terms: ${glossaryTerms.length}`);
  console.log(`   Artifacts: ${visibleArtifacts.length} / ${artifacts.length}`);
...
`;
  ...
  assertCapabilityMapping();
  syncCounts(totalLessons, visiblePhases.length, visibleArtifacts.length);
  syncReadme(totalLessons);
  writeSitemap(visiblePhases, glossaryTerms.length);
  writeLlms(visiblePhases, glossaryTerms.length, visibleArtifacts.length);
}

// Fail the build if the hand-maintained capability→phase map in capabilities.js
// ever points at a hidden phase — that would deep-link to an empty catalog.
function assertCapabilityMapping() {
  const capPath = path.join(__dirname, 'capabilities.js');
  if (!fs.existsSync(capPath)) return;
  const src = fs.readFileSync(capPath, 'utf8');
  const m = src.match(/const CAPABILITIES = (\[[\s\S]*?\n\]);/);
  if (!m) return;
  let caps;
  try { caps = JSON.parse(m[1]); } catch (e) { return; }
  const bad = [];
  caps.forEach(c => (c.phases || []).forEach(pid => {
    if (HIDDEN_PHASES.has(pid)) bad.push(`capability ${c.id} (${c.title}) → hidden phase ${pid}`);
  }));
  if (bad.length) {
    throw new Error('capabilities.js maps to hidden phases:\n  ' + bad.join('\n  '));
  }
}
...
function writeLlms(phases, glossaryCount, artifactCount) {
  ...
  let out = `# LHIND AI Learning Catalog\n\n`;
