...
- Derive the mechanism behind Attention Variants — Sliding Window, Sparse, Differential from tensor operations
...
- Evaluate the computational and modeling trade-offs introduced by Attention Variants — Sliding Window, Sparse, Differential
...
Call `causal_mask` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-attention-variant-picker.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
Use `causal_mask` as the trace: start from tokens=["red","fox"], keep the raw output, and tie each observation to a named objective.

1. **Reproduce the reference path.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `causal_mask`, `swa_mask`, `strided_mask`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Derive the mechanism behind Attention Variants — Sliding Window, Sparse, Differential from tensor operations**.
...
4. **Package a usable handoff.** Open `outputs/skill-attention-variant-picker.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Evaluate the computational and modeling trade-offs introduced by Attention Variants — Sliding Window, Sparse, Differential**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Attention Variants — Sliding Window, Sparse, Differential** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `causal_mask`, `swa_mask`, `strided_mask` traced to the value or shape that supports **Derive the mechanism behind Attention Variants — Sliding Window, Sparse, Differential from tensor operations**;
...
- an updated `outputs/skill-attention-variant-picker.md` example with a concrete input, expected output field, and acceptance check tied to **Evaluate the computational and modeling trade-offs introduced by Attention Variants — Sliding Window, Sparse, Differential**.
