...
Call `sine_wave` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-asr-configurator.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
Work from the smallest fixture that the Audio Transformers — Whisper Architecture demo already understands, then make one deliberate change and record what moved.

1. **Run the smallest fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `sine_wave`, `frame_signal`, `frame_energy`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Derive the mechanism behind Audio Transformers — Whisper Architecture from tensor operations**.
...
4. **Make the result repeatable.** Open `outputs/skill-asr-configurator.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Evaluate the computational and modeling trade-offs introduced by Audio Transformers — Whisper Architecture**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Audio Transformers — Whisper Architecture** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `sine_wave`, `frame_signal`, `frame_energy` traced to the value or shape that supports **Derive the mechanism behind Audio Transformers — Whisper Architecture from tensor operations**;
...
- an updated `outputs/skill-asr-configurator.md` example with a concrete input, expected output field, and acceptance check tied to **Evaluate the computational and modeling trade-offs introduced by Audio Transformers — Whisper Architecture**.
