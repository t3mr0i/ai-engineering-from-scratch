...
Call `MiniBPE` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
Work from the smallest fixture that the Tokenized Dataset with Sliding Window demo already understands, then make one deliberate change and record what moved.

1. **Run the smallest fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `MiniBPE`, `vocab_size`, `initialize`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Convert a raw corpus into a stream of token ids by calling the tokenizer once.**.
2. **Perturb one field.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Slice the id stream into fixed-length windows with a configurable overlap stride.** says the other inputs should stay fixed.
3. **Check the failure boundary.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Build a PyTorch Dataset that returns input and target tensors for next-token prediction.** and record the exception text if the code rejects the case.
4. **Make the result repeatable.** Open `outputs/artifact-card.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Wrap the dataset in a DataLoader with a deterministic shuffle seeded per epoch.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Tokenized Dataset with Sliding Window** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `MiniBPE`, `vocab_size`, `initialize` traced to the value or shape that supports **Convert a raw corpus into a stream of token ids by calling the tokenizer once.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Slice the id stream into fixed-length windows with a configurable overlap stride.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Build a PyTorch Dataset that returns input and target tensors for next-token prediction.**; and
- an updated `outputs/artifact-card.md` example with a concrete input, expected output field, and acceptance check tied to **Wrap the dataset in a DataLoader with a deterministic shuffle seeded per epoch.**.
