...
Call `MultiHeadSelfAttention` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
This lab follows `MultiHeadSelfAttention` and `forward` on a controlled fixture; write down the value before changing the input.

1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `MultiHeadSelfAttention`, `forward`, `TokenEmbedding`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Implement a batched Query/Key/Value projection as a single linear layer split into H heads.**.
2. **Change the controlled parameter.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Compute scaled dot-product attention with the correct normalization and dtype handling.** says the other inputs should stay fixed.
3. **Exercise the guard.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Apply a causal mask that prevents a position from attending to future positions.** and record the exception text if the code rejects the case.
4. **Prepare the artifact for reuse.** Open `outputs/artifact-card.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Inspect per-head attention weights for a fixed input and reason about what each head looks at.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Multi-Head Self-Attention** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `MultiHeadSelfAttention`, `forward`, `TokenEmbedding` traced to the value or shape that supports **Implement a batched Query/Key/Value projection as a single linear layer split into H heads.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Compute scaled dot-product attention with the correct normalization and dtype handling.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Apply a causal mask that prevents a position from attending to future positions.**; and
- an updated `outputs/artifact-card.md` example with a concrete input, expected output field, and acceptance check tied to **Inspect per-head attention weights for a fixed input and reason about what each head looks at.**.
