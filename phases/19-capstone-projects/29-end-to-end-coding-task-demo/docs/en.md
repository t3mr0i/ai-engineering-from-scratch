...
1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `call`, `text`, `usage`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Compose the gate chain, sandbox, eval harness, and span builder into a single agent loop.**.
2. **Change the controlled parameter.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Implement a deterministic policy that uses read_file, run_tests, and write_file to fix a fixture bug.** says the other inputs should stay fixed.
3. **Exercise the guard.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Enforce a global step budget plus an observation token budget across an end-to-end run.** and record the exception text if the code rejects the case.
4. **Prepare the artifact for reuse.** Open `outputs/artifact-card.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Emit complete OTel GenAI traces and Prometheus metrics for the full run.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **End-to-End Coding Agent on the Harness** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `call`, `text`, `usage` traced to the value or shape that supports **Compose the gate chain, sandbox, eval harness, and span builder into a single agent loop.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Implement a deterministic policy that uses read_file, run_tests, and write_file to fix a fixture bug.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Enforce a global step budget plus an observation token budget across an end-to-end run.**; and
- an updated `outputs/artifact-card.md` example with a concrete input, expected output field, and acceptance check tied to **Emit complete OTel GenAI traces and Prometheus metrics for the full run.**.
