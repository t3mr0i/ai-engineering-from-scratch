...
Reconstruct **Capstone 07 — End-to-End Fine-Tuning Pipeline (Data to SFT to DPO to Serve)** by following `Artifact` on a graph with edges (0,1) and (1,2). Run `python3 main.py` and verify that degrees, adjacency, or connectivity expose the isolated/no-edge case explicitly.

## Use It

Call `Artifact` from a small caller with a graph with edges (0,1) and (1,2). Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
Work from the smallest fixture that the Capstone 07 — End-to-End Fine-Tuning Pipeline (Data to SFT to DPO to Serve) demo already understands, then make one deliberate change and record what moved.

1. **Run the smallest fixture.** From `code/`, run `python3 main.py` using a graph with edges (0,1) and (1,2). Follow `Artifact`, `content_hash`, `Manifest`. Expect degrees, adjacency, or connectivity expose the isolated/no-edge case explicitly; capture the first printed shape, metric, status, or summary field and state which part supports **Define measurable acceptance criteria for Capstone 07 — End-to-End Fine-Tuning Pipeline (Data to SFT to DPO to Serve)**.
...
4. **Make the result repeatable.** Open `outputs/skill-finetuning-pipeline.md` and add a worked example using a graph with edges (0,1) and (1,2). Include the input contract, one expected output field, and a named acceptance check for **Package the verified result as a reusable curriculum artifact**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Capstone 07 — End-to-End Fine-Tuning Pipeline (Data to SFT to DPO to Serve)** should contain:

- the `python3 main.py` output for a graph with edges (0,1) and (1,2), with `Artifact`, `content_hash`, `Manifest` traced to the value or shape that supports **Define measurable acceptance criteria for Capstone 07 — End-to-End Fine-Tuning Pipeline (Data to SFT to DPO to Serve)**;
...
- an updated `outputs/skill-finetuning-pipeline.md` example with a concrete input, expected output field, and acceptance check tied to **Package the verified result as a reusable curriculum artifact**.
