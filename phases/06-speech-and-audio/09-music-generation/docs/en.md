...
Call `chord_progression` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-music-designer.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
Work from the smallest fixture that the Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake demo already understands, then make one deliberate change and record what moved.

1. **Run the smallest fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `chord_progression`, `drum_pattern`, `fake_generate`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Explain the signal-processing and modeling concepts behind Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake**.
...
4. **Make the result repeatable.** Open `outputs/skill-music-designer.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Evaluate quality, latency, and robustness trade-offs for Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `chord_progression`, `drum_pattern`, `fake_generate` traced to the value or shape that supports **Explain the signal-processing and modeling concepts behind Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake**;
...
- an updated `outputs/skill-music-designer.md` example with a concrete input, expected output field, and acceptance check tied to **Evaluate quality, latency, and robustness trade-offs for Music Generation — MusicGen, Stable Audio, Suno, and the Licensing Earthquake**.
