...
Call `rms_dbfs` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-realtime-pipeline.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
This lab follows `rms_dbfs` and `simulate_chunk` on a controlled fixture; write down the value before changing the input.

1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `rms_dbfs`, `simulate_chunk`, `vad`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Explain the signal-processing and modeling concepts behind Real-Time Audio Processing**.
...
4. **Prepare the artifact for reuse.** Open `outputs/skill-realtime-pipeline.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Evaluate quality, latency, and robustness trade-offs for Real-Time Audio Processing**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Real-Time Audio Processing** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `rms_dbfs`, `simulate_chunk`, `vad` traced to the value or shape that supports **Explain the signal-processing and modeling concepts behind Real-Time Audio Processing**;
...
- an updated `outputs/skill-realtime-pipeline.md` example with a concrete input, expected output field, and acceptance check tied to **Evaluate quality, latency, and robustness trade-offs for Real-Time Audio Processing**.
