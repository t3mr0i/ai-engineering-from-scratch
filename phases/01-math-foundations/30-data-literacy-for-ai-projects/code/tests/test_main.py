...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "workflow has stale data", ("stale data",), impact=3, uncertainty=2)
        self.assertIn("stale data", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "sensitive field", ("sensitive field",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "stale data", ("stale data",), impact=5, uncertainty=5),
        ]
