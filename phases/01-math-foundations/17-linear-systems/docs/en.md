...
Reconstruct **Linear Systems** by following `gaussian_elimination` on an 8x8 synthetic image. Run `python3 main.py` and verify that the reported height/width or feature-map shape changes predictably, without inventing pixels.

## Use It

Call `gaussian_elimination` from a small caller with an 8x8 synthetic image. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/prompt-linear-solver.md` with the command `python3 main.py`, the accepted input shape (an 8x8 synthetic image), the expected observable result, and a failure note for malformed inputs.
...
Use `gaussian_elimination` as the trace: start from an 8x8 synthetic image, keep the raw output, and tie each observation to a named objective.

1. **Reproduce the reference path.** From `code/`, run `python3 main.py` using an 8x8 synthetic image. Follow `gaussian_elimination`, `lu_decompose`, `lu_solve`. Expect the reported height/width or feature-map shape changes predictably, without inventing pixels; capture the first printed shape, metric, status, or summary field and state which part supports **Solve Ax = b using Gaussian elimination with partial pivoting and back substitution**.
2. **Vary one named input.** Repeat the command after changing only the center-pixel value: use the same image with one bright center pixel. Predict the direction of the change, then compare the two output values. Explain why **Factor matrices with LU, QR, and Cholesky decompositions and explain when each is appropriate** says the other inputs should stay fixed.
3. **Probe the empty case.** Feed the implementation a 1x1 image with all values zero. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Derive the normal equations for least squares and connect them to linear and ridge regression** and record the exception text if the code rejects the case.
4. **Package a usable handoff.** Open `outputs/prompt-linear-solver.md` and add a worked example using an 8x8 synthetic image. Include the input contract, one expected output field, and a named acceptance check for **Diagnose ill-conditioned systems using the condition number and apply regularization to stabilize them**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Linear Systems** should contain:

- the `python3 main.py` output for an 8x8 synthetic image, with `gaussian_elimination`, `lu_decompose`, `lu_solve` traced to the value or shape that supports **Solve Ax = b using Gaussian elimination with partial pivoting and back substitution**;
- a before/after comparison for the center-pixel value, where the same image with one bright center pixel changes the observation in the direction predicted by **Factor matrices with LU, QR, and Cholesky decompositions and explain when each is appropriate**;
- a recorded result for a 1x1 image with all values zero that matches the implementation’s validation or empty-result contract and explains the evidence for **Derive the normal equations for least squares and connect them to linear and ridge regression**; and
- an updated `outputs/prompt-linear-solver.md` example with a concrete input, expected output field, and acceptance check tied to **Diagnose ill-conditioned systems using the condition number and apply regularization to stabilize them**.
