...
The default Python command never opens a network connection. A live call is deliberately opt-in: `ANTHROPIC_LIVE=1`, `ANTHROPIC_API_KEY`, and an explicit provider `ANTHROPIC_MODEL` must all be present. The local `lesson-fixture` label is never accepted as a live model. The TypeScript companion follows the same rule with `LIVE=1`, `ANTHROPIC_API_KEY`, and `ANTHROPIC_MODEL`; without a valid model variable both implementations stay on the fixture and do not open a connection. Neither path requires a provider SDK.
...
`build_request` rejects an empty prompt, empty model, or non-positive token limit. `build_headers` refuses an empty key. `call_raw_http` also rejects the local `lesson-fixture` model, so a real `ANTHROPIC_MODEL` is required before any HTTP opener is called. It is the only function that opens `https://api.anthropic.com/v1/messages`, and the main program reaches it only after all three live variables are present.
...
tmp_dir=$(mktemp -d)
printf '%s\n' 'ANTHROPIC_API_KEY=mock' 'ANTHROPIC_MODEL=example-model' > "$tmp_dir/.env"
(cd "$tmp_dir" && MOCK=1 npx --no-install tsx "$ts_file")
rm -rf "$tmp_dir"
```

It prints the same response shape without contacting a provider. `process.env` takes precedence over the temporary file. If you choose a live experiment, set `LIVE=1`, `ANTHROPIC_API_KEY`, and `ANTHROPIC_MODEL` explicitly, then record the status and redacted body; never paste the key into a command, artifact, or commit.
...
4. Use the temporary-directory TypeScript command above and compare `.env` precedence with `ANTHROPIC_API_KEY` and `ANTHROPIC_MODEL` supplied in the process environment. Keep the file outside the repository and remove that exact temporary directory.

## Reference Solution

A correct local run shows `lesson-fixture`, `64`, one user message, `mode: MOCK`, and the fixed response/usage envelope with exit status 0. The request builder rejects malformed local inputs; the header builder requires a key but never prints it. A live acceptance record is separate and requires `ANTHROPIC_LIVE=1`, a real key, a nonempty `ANTHROPIC_MODEL`, the documented headers/body, and captured provider status; the fixture alone proves only local plumbing.
