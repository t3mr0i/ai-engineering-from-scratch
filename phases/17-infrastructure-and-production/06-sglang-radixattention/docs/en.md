...
Reconstruct **SGLang and RadixAttention for Prefix-Heavy Workloads** by following `token_count` on tokens=["red","fox"]. Run `python3 main.py` and verify that the attention/embedding shape follows the token count and each valid attention row remains normalized.

## Use It

Call `token_count` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
Keep two runs side by side for **SGLang and RadixAttention for Prefix-Heavy Workloads**. The important evidence is the named field, shape, or status—not a polished paragraph about the run.

1. **Read the first result.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `token_count`, `Request`, `RadixCache`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Diagram RadixAttention: how prefixes are stored in a radix tree and how KV blocks are shared across sequences rooted at the same branch.**.
2. **Run a two-value comparison.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Explain cache-aware scheduling and why FCFS is wrong for prefix-heavy traffic.** says the other inputs should stay fixed.
3. **Try an adversarial fixture.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Compute expected speedup for a workload given prefix-cache hit rate and prompt length distribution.** and record the exception text if the code rejects the case.
4. **Write the operator note.** Open `outputs/skill-radix-scheduler-advisor.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Name the prompt-ordering discipline that makes the 6.4x number real vs a lost upside.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **SGLang and RadixAttention for Prefix-Heavy Workloads** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `token_count`, `Request`, `RadixCache` traced to the value or shape that supports **Diagram RadixAttention: how prefixes are stored in a radix tree and how KV blocks are shared across sequences rooted at the same branch.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Explain cache-aware scheduling and why FCFS is wrong for prefix-heavy traffic.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Compute expected speedup for a workload given prefix-cache hit rate and prompt length distribution.**; and
- an updated `outputs/skill-radix-scheduler-advisor.md` example with a concrete input, expected output field, and acceptance check tied to **Name the prompt-ordering discipline that makes the 6.4x number real vs a lost upside.**.
