...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "workflow includes missing owner", ("missing owner",), impact=3, uncertainty=2)
        self.assertIn("missing owner", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "missing owner", ("missing owner",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "missing owner", ("missing owner",), impact=5, uncertainty=5),
        ]
