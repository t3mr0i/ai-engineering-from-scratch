"""AI Business Analysis and Process Discovery artifact.

Lesson docs: phases/11-llm-engineering/56-ai-business-analysis-process-discovery/docs/en.md
...
SIGNALS = ["stakeholder gap", "process variant", "requirement ambiguity", "evidence missing"]
CONTROLS = ["interview guide", "process variant map", "assumption log", "evidence register"]
CATEGORIES = ["stakeholder discovery", "process discovery", "requirement clarification", "evidence review"]
...
def score_scenario(scenario: Scenario) -> int:
    ...
    base = scenario.business_impact * 2 + scenario.ambiguity
    return min(20, base + len(matches) * 2)


def priority_for(score: int) -> str:
    if score >= 16:
        return "discovery workshop"
    if score >= 11:
...
def choose_category(scenario: Scenario) -> str:
    ...
    if not matches:
        return CATEGORIES[0]
    return CATEGORIES[(len(matches) + scenario.business_impact + scenario.ambiguity) % len(CATEGORIES)]
...
def recommend(scenario: Scenario) -> Recommendation:
    ...
    rationale = (
        ...
        f"Business impact={scenario.business_impact}, ambiguity={scenario.ambiguity}."
    )
...
def demo_scenarios() -> list[Scenario]:
    return [
        Scenario("claim workflow", "Stakeholder gap and process variant create requirement ambiguity.", ("stakeholder gap", "process variant", "requirement ambiguity"), 5, 5),
        Scenario("approval flow", "Evidence missing for current approval routing.", ("evidence missing",), 4, 3),
        Scenario("small report", "Stable report change with clear owner.", (), 2, 1),
    ]
