...
The responsible AI conversation shifted significantly between 2024 and 2026. The EU AI Act moved from compliance preparation to partial enforcement — prohibited-use rules and GPAI obligations already apply; high-risk obligations still phase in through 2027-2028. The key practitioner obligations for consulting work in EU-operating clients:
...
Reconstruct **How AI Systems Actually Work: A Consultant's Map of the Four Layers (2026)** by following `SystemType` on the demo’s smallest built-in fixture. Run `python3 main.py` and verify that the result reports the empty case explicitly or raises the documented validation error.

## Use It

Call `SystemType` from a small caller with the demo’s smallest built-in fixture. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
1. **Reproduce the control run.** Run [main.py](../code/main.py) with `python3 main.py` from the lesson's `code/` directory. Record the smallest input that demonstrates “Explain the production problem addressed by How AI Systems Actually Work: A Consultant's Map of the Four Layers (2026)”. Point to `classify_system_type()`, `evaluate_agent_risk()`, `run_failure_case()` and name the returned field or printed value that serves as evidence.
...
4. **Transfer the result.** Open outputs/skill-ai-system-type-classifier.md and adapt one example to a real workflow. State the owner, evidence, and next decision required for “Identify failure modes and define a safe rollback or review path”; mark any assumption that the demo does not establish.

## Reference Solution

A useful submission records python3 main.py, the observed output, and the conclusion drawn from it. It should contain:

- evidence for “Explain the production problem addressed by How AI Systems Actually Work: A Consultant's Map of the Four Layers (2026)” with the relevant input and returned field;
...
- one concrete update to outputs/skill-ai-system-type-classifier.md that applies “Identify failure modes and define a safe rollback or review path” without hiding uncertainty.

Use classify_system_type(), evaluate_agent_risk(), run_failure_case() to explain the result, not only the prose output. If the experiment disagrees with the prediction, keep the failed prediction in the receipt and revise the explanation rather than changing the input until it passes.
