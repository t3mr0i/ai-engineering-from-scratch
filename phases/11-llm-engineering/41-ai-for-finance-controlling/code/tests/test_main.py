...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "source mismatch appears", ("source mismatch",), impact=3, uncertainty=2)
        self.assertIn("source mismatch", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "approval risk", ("approval risk",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "source mismatch", ("source mismatch",), impact=5, uncertainty=5),
        ]
