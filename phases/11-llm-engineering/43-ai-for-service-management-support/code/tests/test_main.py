...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "high ticket volume exists", ("high ticket volume",), impact=3, uncertainty=2)
        self.assertIn("high ticket volume", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "escalation risk", ("escalation risk",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "high ticket volume", ("high ticket volume",), impact=5, uncertainty=5),
        ]
