...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "risk includes prompt injection", ("prompt injection",), impact=3, uncertainty=2)
        self.assertIn("prompt injection", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "unsafe tool", ("unsafe tool",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "prompt injection", ("prompt injection",), impact=5, uncertainty=5),
        ]
