"""AI Security and Prompt Injection Defense course artifact.

Lesson docs: phases/11-llm-engineering/35-ai-security-prompt-injection/docs/en.md
...
SIGNALS = ["prompt injection", "sensitive data", "unsafe tool", "missing audit"]
CONTROLS = ["trust boundary map", "allowlist", "human approval", "audit log"]
CATEGORIES = ["input", "data", "tool", "audit"]
...
def demo_scenarios() -> list[Scenario]:
    return [
        Scenario("tool-enabled assistant", "Assistant uses an unsafe tool and has missing audit evidence.", ("unsafe tool", "missing audit"), 5, 4),
        Scenario("document Q&A", "Retrieved documents may contain prompt injection and sensitive data.", ("prompt injection", "sensitive data"), 4, 4),
        Scenario("training sandbox", "Low-risk awareness exercise with no tool access.", (), 2, 2),
    ]
