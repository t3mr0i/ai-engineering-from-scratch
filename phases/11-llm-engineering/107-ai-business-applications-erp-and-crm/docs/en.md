...
Reconstruct **AI Use-Case Evaluation in ERP and CRM Systems (2026)** by following `RAG` on the demo’s smallest built-in fixture. Run `python3 main.py` and verify that the result reports the empty case explicitly or raises the documented validation error.

## Use It

Call `RAG` from a small caller with the demo’s smallest built-in fixture. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
4. **Transfer the result.** Open outputs/skill-erp-crm-use-case-evaluator.md and adapt one example to a real workflow. State the owner, evidence, and next decision required for “Identify failure modes and define a safe rollback or review path”; mark any assumption that the demo does not establish.

## Reference Solution

A useful submission records python3 main.py, the observed output, and the conclusion drawn from it. It should contain:

- evidence for “Explain the production problem addressed by AI Use-Case Evaluation in ERP and CRM Systems (2026)” with the relevant input and returned field;
...
- one concrete update to outputs/skill-erp-crm-use-case-evaluator.md that applies “Identify failure modes and define a safe rollback or review path” without hiding uncertainty.

Use classify_system_boundary(), classify_data_ownership(), classify_integration_pattern() to explain the result, not only the prose output. If the experiment disagrees with the prediction, keep the failed prediction in the receipt and revise the explanation rather than changing the input until it passes.
