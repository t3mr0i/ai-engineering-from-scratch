...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "capacity conflict exists", ("capacity conflict",), impact=3, uncertainty=2)
        self.assertIn("capacity conflict", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "weak metric", ("weak metric",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "capacity conflict", ("capacity conflict",), impact=5, uncertainty=5),
        ]
