"""AI Service Desk Runbook and Knowledge Automation artifact.

Lesson docs: phases/11-llm-engineering/65-ai-service-desk-runbook-automation/docs/en.md
Source basis: LHIND AI Self-Assessment capability and training catalog.
Implements the reusable classroom artifact without external dependencies.
Run with: python3 main.py
"""

from __future__ import annotations

import json
from dataclasses import dataclass
from typing import Iterable


TITLE = "AI Service Desk Runbook and Knowledge Automation"
CAPABILITY = "Application Management - AI Service Runbook Fit"
SIGNALS = ["repeat ticket", "known fix", "escalation rule", "knowledge gap"]
CONTROLS = ["runbook step", "source article", "handoff trigger", "quality feedback"]
CATEGORIES = ["self-service answer", "agent assist", "runbook automation", "escalation review"]


@dataclass(frozen=True)
class Scenario:
    name: str
    description: str
    signals: tuple[str, ...]
    ticket_volume: int = 3
    service_risk: int = 3


@dataclass(frozen=True)
class Recommendation:
    category: str
    score: int
    priority: str
    controls: tuple[str, ...]
    rationale: str


def normalize(text: str) -> str:
    return " ".join(text.lower().replace("-", " ").split())


def signal_matches(scenario: Scenario) -> list[str]:
    haystack_words = normalize(" ".join((scenario.name, scenario.description, " ".join(scenario.signals)))).split()
    matches = []
    for signal in SIGNALS:
        words = normalize(signal).split()
        if all(word in haystack_words for word in words):
            matches.append(signal)
        elif any(word in haystack_words for word in words):
            matches.append(signal)
    return matches


def score_scenario(scenario: Scenario) -> int:
    matches = signal_matches(scenario)
    base = scenario.ticket_volume * 2 + scenario.service_risk
    return min(20, base + len(matches) * 2)


def priority_for(score: int) -> str:
    if score >= 16:
        return "pilot runbook automation"
    if score >= 11:
        return "design agent assist"
    if score >= 7:
        return "improve knowledge article"
    return "observe"


def choose_category(scenario: Scenario) -> str:
    matches = signal_matches(scenario)
    if not matches:
        return CATEGORIES[0]
    return CATEGORIES[(len(matches) + scenario.ticket_volume + scenario.service_risk) % len(CATEGORIES)]


def recommend(scenario: Scenario) -> Recommendation:
    matches = signal_matches(scenario)
    score = score_scenario(scenario)
    selected_controls = tuple(CONTROLS[: max(2, min(len(CONTROLS), 1 + len(matches)))])
    rationale = (
        f"Matched {len(matches)} signal(s): {', '.join(matches) if matches else 'none'}. "
        f"Ticket volume={scenario.ticket_volume}, service risk={scenario.service_risk}."
    )
    return Recommendation(choose_category(scenario), score, priority_for(score), selected_controls, rationale)


def build_plan(scenarios: Iterable[Scenario]) -> list[dict]:
    rows = []
    for scenario in scenarios:
        rec = recommend(scenario)
        rows.append({
            "scenario": scenario.name,
            "category": rec.category,
            "score": rec.score,
            "priority": rec.priority,
            "controls": list(rec.controls),
            "rationale": rec.rationale,
        })
    return sorted(rows, key=lambda row: row["score"], reverse=True)


def demo_scenarios() -> list[Scenario]:
    return [
        Scenario("vpn password reset", "Repeat ticket with a known fix and source article gap.", ("repeat ticket", "known fix", "knowledge gap"), 5, 3),
        Scenario("erp outage handoff", "Escalation rule is unclear for a high-risk service.", ("escalation rule",), 3, 5),
        Scenario("one-off request", "Unusual request without a reusable article.", (), 1, 1),
    ]


def main() -> None:
    print(json.dumps({"title": TITLE, "capability": CAPABILITY, "plan": build_plan(demo_scenarios())}, indent=2))


if __name__ == "__main__":
    main()
