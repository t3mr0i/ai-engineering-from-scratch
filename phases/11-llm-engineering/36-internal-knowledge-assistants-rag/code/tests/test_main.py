...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "source sprawl is visible", ("source sprawl",), impact=3, uncertainty=2)
        self.assertIn("source sprawl", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "permission risk", ("permission risk",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "source sprawl", ("source sprawl",), impact=5, uncertainty=5),
        ]
