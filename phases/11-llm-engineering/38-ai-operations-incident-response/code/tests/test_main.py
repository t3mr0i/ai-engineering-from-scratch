...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "quality drift detected", ("quality drift",), impact=3, uncertainty=2)
        self.assertIn("quality drift", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "cost spike", ("cost spike",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "quality drift", ("quality drift",), impact=5, uncertainty=5),
        ]
