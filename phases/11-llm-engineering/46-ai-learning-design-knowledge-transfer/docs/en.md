...
## CBP context: behavior change, not training volume

In the CBP context learning design starts from the behavior to change — role, skill gap, practice task, evidence of transfer — and uses AI to draft around that spine, never to generate volume. Knowledge checks measure on-the-job behavior, not course completion clicks. Training that cannot show transfer is content production, not enablement.
