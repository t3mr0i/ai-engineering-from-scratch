...
Reconstruct **Few-Shot, Chain-of-Thought, Tree-of-Thought** by following `extract_answer` on the text "red fox". Run `python3 main.py` and verify that the tokenizer/retriever reports zero or a clear empty-input result, rather than borrowing a result from the previous text.

## Use It

Call `extract_answer` from a small caller with the text "red fox". Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
1. **Trace the happy path.** Run [`main.py`](../code/main.py) with `python3 main.py` from the lesson's `code/` directory. Record the smallest input that demonstrates “Implement few-shot prompting by selecting and formatting example demonstrations that maximize task accuracy”. Point to `main.py` and name the returned field or printed value that serves as evidence.
2. **Perturb the input.** Change exactly one input, threshold, or option that affects “Apply chain-of-thought (CoT) reasoning to improve accuracy on multi-step problems like math word problems”. Predict the direction of the change before running it, then compare the two outputs and explain why the other fields should stay stable.
3. **Test a failure case.** Construct a case that stresses “Build a tree-of-thought prompt that explores multiple reasoning paths and selects the best one”: choose an empty collection, missing field, maximum-sized value, malformed record, or another boundary that fits this lesson. Write the expected behavior first and distinguish an intentional guard from an accidental crash.
4. **Transfer the result.** Open `outputs/prompt-reasoning-chain.md` and adapt one example to a real workflow. State the owner, evidence, and next decision required for “Measure the accuracy improvement from zero-shot vs few-shot vs CoT on a standard benchmark”; mark any assumption that the demo does not establish.

## Reference Solution

The reference run should leave a small receipt: `python3 main.py`, its captured output, and your interpretation. Include:

- evidence for “Implement few-shot prompting by selecting and formatting example demonstrations that maximize task accuracy” with the relevant input and returned field;
- a one-variable comparison that makes “Apply chain-of-thought (CoT) reasoning to improve accuracy on multi-step problems like math word problems” visible;
- a predicted and observed boundary result for “Build a tree-of-thought prompt that explores multiple reasoning paths and selects the best one”, including why the behavior is safe; and
- one concrete update to `outputs/prompt-reasoning-chain.md` that applies “Measure the accuracy improvement from zero-shot vs few-shot vs CoT on a standard benchmark” without hiding uncertainty.
