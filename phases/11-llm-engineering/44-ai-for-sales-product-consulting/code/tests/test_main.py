...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "customer signal is weak", ("customer signal",), impact=3, uncertainty=2)
        self.assertIn("customer signal", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "proposal risk", ("proposal risk",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "customer signal", ("customer signal",), impact=5, uncertainty=5),
        ]
