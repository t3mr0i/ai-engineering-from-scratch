...
## CBP context: residency, exit, evidence before signature

In the CBP context procurement reviews check data residency, Entra ID integration, audit evidence, per-tenant pricing behavior, and the exit strategy before any signature — the demo is the smallest part of the decision. Azure OpenAI is the default precisely because these answers exist; challengers provide them in writing with CBP evals attached. A fast decision without them is long-term risk with a discount.
