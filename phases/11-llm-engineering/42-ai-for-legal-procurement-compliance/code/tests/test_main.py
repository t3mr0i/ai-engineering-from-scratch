...
class CourseArtifactTests(unittest.TestCase):
    def test_signal_matching_finds_known_signal(self):
        scenario = Scenario("pilot", "vendor clause is present", ("vendor clause",), impact=3, uncertainty=2)
        self.assertIn("vendor clause", signal_matches(scenario))
    ...
    def test_recommendation_contains_controls(self):
        scenario = Scenario("controlled", "obligation risk", ("obligation risk",), impact=4, uncertainty=4)
    ...
    def test_build_plan_sorts_by_score_descending(self):
        scenarios = [
            ...
            Scenario("large", "vendor clause", ("vendor clause",), impact=5, uncertainty=5),
        ]
