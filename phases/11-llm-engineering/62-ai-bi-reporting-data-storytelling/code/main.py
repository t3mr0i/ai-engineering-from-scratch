"""AI BI Reporting and Data Storytelling artifact.

Lesson docs: phases/11-llm-engineering/62-ai-bi-reporting-data-storytelling/docs/en.md
...
SIGNALS = ["metric ambiguity", "visualization risk", "causality claim", "audience decision"]
CONTROLS = ["metric definition", "chart check", "causality warning", "decision context"]
CATEGORIES = ["dashboard narrative", "management report", "metric review", "decision briefing"]
...
def score_scenario(scenario: Scenario) -> int:
    ...
    base = scenario.decision_impact * 2 + scenario.evidence_risk
    return min(20, base + len(matches) * 2)


def priority_for(score: int) -> str:
    if score >= 16:
        return "report review required"
    if score >= 11:
        return "guided data story"
    if score >= 7:
        return "metric check"
    return "self check"
...
def choose_category(scenario: Scenario) -> str:
    ...
    if not matches:
        return CATEGORIES[0]
    return CATEGORIES[(len(matches) + scenario.decision_impact + scenario.evidence_risk) % len(CATEGORIES)]
...
def recommend(scenario: Scenario) -> Recommendation:
    ...
    rationale = (
        ...
        f"Decision impact={scenario.decision_impact}, evidence risk={scenario.evidence_risk}."
    )
...
def demo_scenarios() -> list[Scenario]:
    return [
        Scenario("executive kpi story", "Audience decision depends on ambiguous metric and causality claim.", ("metric ambiguity", "causality claim", "audience decision"), 5, 5),
        Scenario("dashboard refresh", "Visualization risk from a changed chart scale.", ("visualization risk",), 3, 4),
        Scenario("team usage snapshot", "Low-risk operational chart.", (), 2, 1),
    ]
