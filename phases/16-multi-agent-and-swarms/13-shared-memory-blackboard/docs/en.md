...
- Explain the coordination mechanism behind Shared Memory and Blackboard Patterns
...
Call `ProvenanceEntry` from a small caller with the demo’s smallest built-in fixture. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-memory-auditor.md` with the command `python3 main.py`, the accepted input shape (the demo’s smallest built-in fixture), the expected observable result, and a failure note for malformed inputs.
...
Keep two runs side by side for **Shared Memory and Blackboard Patterns**. The important evidence is the named field, shape, or status—not a polished paragraph about the run.

1. **Read the first result.** From `code/`, run `python3 main.py` using the demo’s smallest built-in fixture. Follow `ProvenanceEntry`, `MessagePool`, `write`. Expect the result reports the empty case explicitly or raises the documented validation error; capture the first printed shape, metric, status, or summary field and state which part supports **Explain the coordination mechanism behind Shared Memory and Blackboard Patterns**.
...
4. **Write the operator note.** Open `outputs/skill-memory-auditor.md` and add a worked example using the demo’s smallest built-in fixture. Include the input contract, one expected output field, and a named acceptance check for **Evaluate coordination quality, cost, and correlated failure modes**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Shared Memory and Blackboard Patterns** should contain:

- the `python3 main.py` output for the demo’s smallest built-in fixture, with `ProvenanceEntry`, `MessagePool`, `write` traced to the value or shape that supports **Explain the coordination mechanism behind Shared Memory and Blackboard Patterns**;
...
- an updated `outputs/skill-memory-auditor.md` example with a concrete input, expected output field, and acceptance check tied to **Evaluate coordination quality, cost, and correlated failure modes**.
