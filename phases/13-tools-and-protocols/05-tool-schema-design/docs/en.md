...
Reconstruct **Tool Schema Design — Naming, Descriptions, Parameter Constraints** by following `Finding` on x=0.5 with the demo defaults. Run `python3 main.py` and verify that the update or loss change agrees with the gradient sign; a zero gradient produces no accidental jump.
...
Keep two runs side by side for **Tool Schema Design — Naming, Descriptions, Parameter Constraints**. The important evidence is the named field, shape, or status—not a polished paragraph about the run.

1. **Read the first result.** From `code/`, run `python3 main.py` using x=0.5 with the demo defaults. Follow `Finding`, `lint_name`, `lint_description`. Expect the update or loss change agrees with the gradient sign; a zero gradient produces no accidental jump; capture the first printed shape, metric, status, or summary field and state which part supports **Write a tool description using the "Use when X. Do not use for Y." pattern, under 1024 characters.**.
2. **Run a two-value comparison.** Repeat the command after changing only the learning rate: use the same run with learning rate 0.1 instead of 0.01. Predict the direction of the change, then compare the two output values. Explain why **Name tools in a way that is stable, `snake_case`, and unambiguous across a large registry.** says the other inputs should stay fixed.
3. **Try an adversarial fixture.** Feed the implementation a zero gradient or an already-minimized point. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Choose between atomic tools and a single monolithic tool for a given task surface.** and record the exception text if the code rejects the case.
4. **Write the operator note.** Open `outputs/skill-tool-schema-linter.md` and add a worked example using x=0.5 with the demo defaults. Include the input contract, one expected output field, and a named acceptance check for **Run a tool-schema linter against a registry and fix the findings.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Tool Schema Design — Naming, Descriptions, Parameter Constraints** should contain:

- the `python3 main.py` output for x=0.5 with the demo defaults, with `Finding`, `lint_name`, `lint_description` traced to the value or shape that supports **Write a tool description using the "Use when X. Do not use for Y." pattern, under 1024 characters.**;
- a before/after comparison for the learning rate, where the same run with learning rate 0.1 instead of 0.01 changes the observation in the direction predicted by **Name tools in a way that is stable, `snake_case`, and unambiguous across a large registry.**;
- a recorded result for a zero gradient or an already-minimized point that matches the implementation’s validation or empty-result contract and explains the evidence for **Choose between atomic tools and a single monolithic tool for a given task surface.**; and
- an updated `outputs/skill-tool-schema-linter.md` example with a concrete input, expected output field, and acceptance check tied to **Run a tool-schema linter against a registry and fix the findings.**.
