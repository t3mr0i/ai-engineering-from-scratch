...
Reconstruct **Capstone — Build a Complete Tool Ecosystem** by following `call` on the text "red fox". Run `python3 main.py` and verify that the tokenizer/retriever reports zero or a clear empty-input result, rather than borrowing a result from the previous text.
...
Keep two runs side by side for **Capstone — Build a Complete Tool Ecosystem**. The important evidence is the named field, shape, or status—not a polished paragraph about the run.

1. **Read the first result.** From `code/`, run `python3 main.py` using the text "red fox". Follow `call`, `text`, `usage`. Expect the tokenizer/retriever reports zero or a clear empty-input result, rather than borrowing a result from the previous text; capture the first printed shape, metric, status, or summary field and state which part supports **Compose an MCP server exposing tools, resources, prompts, and a task with a `ui://` app.**.
2. **Run a two-value comparison.** Repeat the command after changing only the input text: use the text "red fox runs". Predict the direction of the change, then compare the two output values. Explain why **Front the server with an OAuth 2.1 gateway that enforces RBAC and pinned hashes.** says the other inputs should stay fixed.
3. **Try an adversarial fixture.** Feed the implementation an empty string. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Write a multi-server client that traces with OTel GenAI attributes end-to-end.** and record the exception text if the code rejects the case.
4. **Write the operator note.** Open `outputs/skill-ecosystem-blueprint.md` and add a worked example using the text "red fox". Include the input contract, one expected output field, and a named acceptance check for **Delegate part of a workload to an A2A sub-agent; verify opacity is preserved.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Capstone — Build a Complete Tool Ecosystem** should contain:

- the `python3 main.py` output for the text "red fox", with `call`, `text`, `usage` traced to the value or shape that supports **Compose an MCP server exposing tools, resources, prompts, and a task with a `ui://` app.**;
- a before/after comparison for the input text, where the text "red fox runs" changes the observation in the direction predicted by **Front the server with an OAuth 2.1 gateway that enforces RBAC and pinned hashes.**;
- a recorded result for an empty string that matches the implementation’s validation or empty-result contract and explains the evidence for **Write a multi-server client that traces with OTel GenAI attributes end-to-end.**; and
- an updated `outputs/skill-ecosystem-blueprint.md` example with a concrete input, expected output field, and acceptance check tied to **Delegate part of a workload to an A2A sub-agent; verify opacity is preserved.**.
