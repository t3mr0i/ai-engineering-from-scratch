...
Reconstruct **MCP Auth in Production — Enrollment, JWKS Refresh, Audience-Pinned Tokens** by following `b64url` on tokens=["red","fox"]. Run `python3 main.py` and verify that the attention/embedding shape follows the token count and each valid attention row remains normalized.

## Use It

Call `b64url` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
This lab follows `b64url` and `b64url_decode` on a controlled fixture; write down the value before changing the input.

1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `b64url`, `b64url_decode`, `jwt_sign`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Discover an authorization server through RFC 8414 metadata and verify the contract.**.
2. **Change the controlled parameter.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Implement RFC 7591 dynamic client registration so MCP clients enroll without admin intervention.** says the other inputs should stay fixed.
3. **Exercise the guard.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Cache and refresh JWKS keys on a schedule so signature verification survives key roll-over.** and record the exception text if the code rejects the case.
4. **Prepare the artifact for reuse.** Open `outputs/skill-mcp-auth.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Pin tokens to a single MCP resource using RFC 8707 resource indicators and refuse confused-deputy reuse.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **MCP Auth in Production — Enrollment, JWKS Refresh, Audience-Pinned Tokens** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `b64url`, `b64url_decode`, `jwt_sign` traced to the value or shape that supports **Discover an authorization server through RFC 8414 metadata and verify the contract.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Implement RFC 7591 dynamic client registration so MCP clients enroll without admin intervention.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Cache and refresh JWKS keys on a schedule so signature verification survives key roll-over.**; and
- an updated `outputs/skill-mcp-auth.md` example with a concrete input, expected output field, and acceptance check tied to **Pin tokens to a single MCP resource using RFC 8707 resource indicators and refuse confused-deputy reuse.**.
