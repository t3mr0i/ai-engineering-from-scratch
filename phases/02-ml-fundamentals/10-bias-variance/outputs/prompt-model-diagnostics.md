...
Run `python3 code/main.py` with the recorded degree list, `n_bootstrap`, `n_train`, `n_test`, noise level, and ridge `lam`. Preserve the resulting `bias_sq`, `variance`, `reducible_error`, `noise`, and `total_error` columns. Check that `total_error == bias_sq + variance + noise`; keep `reducible_error` as the separate empirical noiseless-fit diagnostic.
