...
Call `quantize_patch` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-tokenizer-vs-adapter-picker.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
This lab follows `quantize_patch` and `image_to_tokens` on a controlled fixture; write down the value before changing the input.

1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `quantize_patch`, `image_to_tokens`, `synthesize_caption`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Explain why a shared vocabulary + single loss changes what the model can do.**.
2. **Change the controlled parameter.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Describe how a VQ-VAE tokenizes an image into a discrete sequence compatible with a transformer's next-token objective.** says the other inputs should stay fixed.
3. **Exercise the guard.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Name Chameleon's training-stability tricks: QK-Norm, dropout placement, LayerNorm ordering.** and record the exception text if the code rejects the case.
4. **Prepare the artifact for reuse.** Open `outputs/skill-tokenizer-vs-adapter-picker.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Compare Chameleon vs BLIP-2's Q-Former approach and describe when each is the right choice.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Chameleon and Early-Fusion Token-Only Multimodal Models** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `quantize_patch`, `image_to_tokens`, `synthesize_caption` traced to the value or shape that supports **Explain why a shared vocabulary + single loss changes what the model can do.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Describe how a VQ-VAE tokenizes an image into a discrete sequence compatible with a transformer's next-token objective.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Name Chameleon's training-stability tricks: QK-Norm, dropout placement, LayerNorm ordering.**; and
- an updated `outputs/skill-tokenizer-vs-adapter-picker.md` example with a concrete input, expected output field, and acceptance check tied to **Compare Chameleon vs BLIP-2's Q-Former approach and describe when each is the right choice.**.
