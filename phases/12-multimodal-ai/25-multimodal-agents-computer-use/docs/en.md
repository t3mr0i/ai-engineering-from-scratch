...
Call `BrowserState` from a small caller with the text "red fox". Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-multimodal-agent-designer.md` with the command `python3 main.py`, the accepted input shape (the text "red fox"), the expected observable result, and a failure note for malformed inputs.
...
This lab follows `BrowserState` and `Task` on a controlled fixture; write down the value before changing the input.

1. **Trace the canonical fixture.** From `code/`, run `python3 main.py` using the text "red fox". Follow `BrowserState`, `Task`, `mock_tasks`. Expect the tokenizer/retriever reports zero or a clear empty-input result, rather than borrowing a result from the previous text; capture the first printed shape, metric, status, or summary field and state which part supports **Design a multimodal agent loop: perceive → reason → act → observe → repeat.**.
2. **Change the controlled parameter.** Repeat the command after changing only the input text: use the text "red fox runs". Predict the direction of the change, then compare the two output values. Explain why **Build a GUI grounding output schema (click coordinates, type text, scroll, drag) the VLM can emit as JSON.** says the other inputs should stay fixed.
3. **Exercise the guard.** Feed the implementation an empty string. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Compare screenshot-only agents vs accessibility-tree agents vs hybrid agents.** and record the exception text if the code rejects the case.
4. **Prepare the artifact for reuse.** Open `outputs/skill-multimodal-agent-designer.md` and add a worked example using the text "red fox". Include the input contract, one expected output field, and a named acceptance check for **Set up a multimodal agent benchmark evaluation on a small VisualWebArena slice.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Multimodal Agents and Computer-Use (Capstone)** should contain:

- the `python3 main.py` output for the text "red fox", with `BrowserState`, `Task`, `mock_tasks` traced to the value or shape that supports **Design a multimodal agent loop: perceive → reason → act → observe → repeat.**;
- a before/after comparison for the input text, where the text "red fox runs" changes the observation in the direction predicted by **Build a GUI grounding output schema (click coordinates, type text, scroll, drag) the VLM can emit as JSON.**;
- a recorded result for an empty string that matches the implementation’s validation or empty-result contract and explains the evidence for **Compare screenshot-only agents vs accessibility-tree agents vs hybrid agents.**; and
- an updated `outputs/skill-multimodal-agent-designer.md` example with a concrete input, expected output field, and acceptance check tied to **Set up a multimodal agent benchmark evaluation on a small VisualWebArena slice.**.
