...
Call `VocabSlot` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.

## Ship It

Hand off `outputs/skill-any-to-any-pipeline-auditor.md` with the command `python3 main.py`, the accepted input shape (tokens=["red","fox"]), the expected observable result, and a failure note for malformed inputs.
...
Use `VocabSlot` as the trace: start from tokens=["red","fox"], keep the raw output, and tie each observation to a named objective.

1. **Reproduce the reference path.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `VocabSlot`, `end`, `build_vocab`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Design a shared vocabulary that hosts text, image, speech, and music tokens without collisions.**.
2. **Vary one named input.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Compare SEED-Tokenizer (images) and SpeechTokenizer residual-VQ (speech) on compression + reconstruction trade-offs.** says the other inputs should stay fixed.
3. **Probe the empty case.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Explain the four-stage curriculum that builds up any-to-any generation.** and record the exception text if the code rejects the case.
4. **Package a usable handoff.** Open `outputs/skill-any-to-any-pipeline-auditor.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Name the three open any-to-any recipes and their main trade-offs: MIO, AnyGPT, Unified-IO 2.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **MIO and Any-to-Any Streaming Multimodal Models** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `VocabSlot`, `end`, `build_vocab` traced to the value or shape that supports **Design a shared vocabulary that hosts text, image, speech, and music tokens without collisions.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Compare SEED-Tokenizer (images) and SpeechTokenizer residual-VQ (speech) on compression + reconstruction trade-offs.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Explain the four-stage curriculum that builds up any-to-any generation.**; and
- an updated `outputs/skill-any-to-any-pipeline-auditor.md` example with a concrete input, expected output field, and acceptance check tied to **Name the three open any-to-any recipes and their main trade-offs: MIO, AnyGPT, Unified-IO 2.**.
