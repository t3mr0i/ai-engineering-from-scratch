...
Reconstruct **Multi-Session Handoff** by following `WorkbenchSnapshot` on the demo’s smallest built-in fixture. Run `python3 main.py` and verify that the result reports the empty case explicitly or raises the documented validation error.

## Use It

Call `WorkbenchSnapshot` from a small caller with the demo’s smallest built-in fixture. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
Keep two runs side by side for **Multi-Session Handoff**. The important evidence is the named field, shape, or status—not a polished paragraph about the run.

1. **Read the first result.** From `code/`, run `python3 main.py` using the demo’s smallest built-in fixture. Follow `WorkbenchSnapshot`, `HandoffPayload`, `trim_feedback`. Expect the result reports the empty case explicitly or raises the documented validation error; capture the first printed shape, metric, status, or summary field and state which part supports **Identify the seven fields every handoff packet needs.**.
2. **Run a two-value comparison.** Repeat the command after changing only the primary fixture value: use the same fixture with its primary value changed from 1 to 2. Predict the direction of the change, then compare the two output values. Explain why **Generate a handoff from the workbench artifacts without hand-writing prose.** says the other inputs should stay fixed.
3. **Try an adversarial fixture.** Feed the implementation an empty fixture {}. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Trim large feedback logs into a handoff-sized summary.** and record the exception text if the code rejects the case.
4. **Write the operator note.** Open `outputs/skill-handoff-generator.md` and add a worked example using the demo’s smallest built-in fixture. Include the input contract, one expected output field, and a named acceptance check for **Make the next session's first action deterministic.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Multi-Session Handoff** should contain:

- the `python3 main.py` output for the demo’s smallest built-in fixture, with `WorkbenchSnapshot`, `HandoffPayload`, `trim_feedback` traced to the value or shape that supports **Identify the seven fields every handoff packet needs.**;
- a before/after comparison for the primary fixture value, where the same fixture with its primary value changed from 1 to 2 changes the observation in the direction predicted by **Generate a handoff from the workbench artifacts without hand-writing prose.**;
- a recorded result for an empty fixture {} that matches the implementation’s validation or empty-result contract and explains the evidence for **Trim large feedback logs into a handoff-sized summary.**; and
- an updated `outputs/skill-handoff-generator.md` example with a concrete input, expected output field, and acceptance check tied to **Make the next session's first action deterministic.**.
