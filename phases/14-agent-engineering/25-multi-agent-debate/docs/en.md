...
Reconstruct **Multi-Agent Debate and Collaboration** by following `Debater` on tokens=["red","fox"]. Run `python3 main.py` and verify that the attention/embedding shape follows the token count and each valid attention row remains normalized.

## Use It

Call `Debater` from a small caller with tokens=["red","fox"]. Compare its result with the demo output, and record the input contract and the one field a downstream user should rely on.
...
Work from the smallest fixture that the Multi-Agent Debate and Collaboration demo already understands, then make one deliberate change and record what moved.

1. **Run the smallest fixture.** From `code/`, run `python3 main.py` using tokens=["red","fox"]. Follow `Debater`, `drift`, `full_mesh_round`. Expect the attention/embedding shape follows the token count and each valid attention row remains normalized; capture the first printed shape, metric, status, or summary field and state which part supports **Explain the debate protocol: N proposers, R rounds, converge on a shared answer.**.
2. **Perturb one field.** Repeat the command after changing only the token sequence: use tokens=["red","fox","runs"]. Predict the direction of the change, then compare the two output values. Explain why **Describe why debate improves factuality, rule-following, and reasoning.** says the other inputs should stay fixed.
3. **Check the failure boundary.** Feed the implementation tokens=[]. Before running it, write down whether the relevant function should return an empty value, a zero-sized result, or a validation error. Check the observed status against **Explain sparse topology: not every debater needs to see every other.** and record the exception text if the code rejects the case.
4. **Make the result repeatable.** Open `outputs/skill-debate.md` and add a worked example using tokens=["red","fox"]. Include the input contract, one expected output field, and a named acceptance check for **Implement a stdlib debate over a scripted LLM with full-mesh and sparse variants; measure token cost vs accuracy.**; note what the demo cannot establish.

## Reference Solution

A checkable result for **Multi-Agent Debate and Collaboration** should contain:

- the `python3 main.py` output for tokens=["red","fox"], with `Debater`, `drift`, `full_mesh_round` traced to the value or shape that supports **Explain the debate protocol: N proposers, R rounds, converge on a shared answer.**;
- a before/after comparison for the token sequence, where tokens=["red","fox","runs"] changes the observation in the direction predicted by **Describe why debate improves factuality, rule-following, and reasoning.**;
- a recorded result for tokens=[] that matches the implementation’s validation or empty-result contract and explains the evidence for **Explain sparse topology: not every debater needs to see every other.**; and
- an updated `outputs/skill-debate.md` example with a concrete input, expected output field, and acceptance check tied to **Implement a stdlib debate over a scripted LLM with full-mesh and sparse variants; measure token cost vs accuracy.**.
